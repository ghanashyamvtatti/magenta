...
class NSynthTfdsDataset(BaseDataset):
    ...
    def provide_dataset(self):
        ...
        def _parse_nsynth(example):
            ...
            wave, label = example['audio'], example['pitch'][0]
...
class ReverbSynthDataset(BaseDataset):
    def provide_one_hot_labels(self, batch_size):
        """Provides one hot labels."""
        pitch_counts = self.get_pitch_counts()
        pitches = sorted(pitch_counts.keys())
    ...
    def _get_dataset_from_files(self):
        dataset = tf.data.TFRecordDataset(self._config['tfds_data_dir'])
        dataset = dataset.shuffle(buffer_size=1000)
        dataset = dataset.repeat()
        return dataset

    def provide_dataset(self):
        """Provides dataset (audio, labels) of reverbnsynth."""
        length = 64000
        channels = 1
        ...
        def _parse_reverbsynth(record_batch):
            n_samples = 64000

            # Create a description of the features
            feature_description = {
                'audio': tf.io.FixedLenFeature([n_samples], tf.float32),
                'pitch': tf.io.FixedLenFeature(1, tf.int64),
            }
        ...
        dataset = self._get_dataset_from_files()
        dataset = dataset.map(
            _parse_reverbsynth, num_parallel_calls=tf.data.experimental.AUTOTUNE)
        return dataset
...
registry = {
    ...
    'reverbsynth': ReverbSynthDataset,
}
